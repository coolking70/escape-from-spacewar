...
- `core-v4` 单场 3D 太空战斗；
- V0.6～V0.9 的七层航道兼容战役；
- V1.0「单星域高速 SLG + 星门撤离」主方向（V1.0-D.2 已完成逐舰撤离清单垂直切片）。
...
- 版本：`1.0-alpha.11`（在 D.1 生产闭环上加入按 `campaignShipId` 精确绑定的逐舰撤离清单）；`1.0-alpha.2` 至 `1.0-alpha.10` 存档均会确定性迁移到当前版本
...
`npm run test:strategy` 覆盖 88 项（无 `as unknown as` 伪造 BattleState），其中 C.5 正式矩阵对 65 个 seed 逐步执行远征码往返与六场真实 core-v4 战斗并全部取得三星域胜利；额外 1000-seed 发布探针同样无失败。`npm run test:browser` 使用真实 Chromium 验证星域界面的视口滚动、指挥官招募、真实航行、次级前哨建立、本地建设队列、移动敌军、主基地围攻、舰队回防、D.1 船坞建设与舰船交付、D.2 逐舰撤离任务及预览，以及完整三星域 UI 流程中的六次 Three.js 战斗和胜利结算；也可通过 `BROWSER_TEST_URL` 验收已经启动的单文件静态站点：
...
- alpha.2 的零值、极低值和高值迁移保持确定、合法并按最近可达战力钳制；Sector Expedition Code 当前为 `1.0-alpha.11`。
...
V1.0-D.2 在已冻结的 C.5 三星域闭环上加入逐舰撤离清单：稳定/紧急撤离均以 `campaignShipId` 精确覆盖舰队，预览和结算共享同一纯计划；失能舰拖曳保留组件损伤并产生明确成本，断后/放弃按指定 ID 损失。Sector Expedition Code 为 `1.0-alpha.11`，策略套件现为 88 项，Chromium 已从正式 UI 验证逐舰任务、准确预览和三星域胜利。

下一切片是让已经继承的永久蓝图产生受边界约束的战略效果。舰船模块装配/改装仍未实现；后续也不默认扩展多玩家舰队、基地升级树、实时战略移动、外交或市场。Core-v4 战斗规则、舰船成本、AI、状态语义和黄金回放保持冻结。
