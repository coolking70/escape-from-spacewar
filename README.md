...
- `core-v4` 单场 3D 太空战斗；
- V0.6～V0.9 的七层航道兼容战役；
- V1.0「单星域高速 SLG + 星门撤离」主方向（V1.0-D.3 已完成永久蓝图效果垂直切片）。
...
- 版本：`1.0-alpha.12`（在 D.2 逐舰撤离闭环上统一三个永久蓝图的战略效果）；`1.0-alpha.2` 至 `1.0-alpha.11` 存档均会确定性迁移到当前版本
...
`npm run test:strategy` 覆盖 91 项（无 `as unknown as` 伪造 BattleState），其中 C.5 正式矩阵对 65 个 seed 逐步执行远征码往返与六场真实 core-v4 战斗并全部取得三星域胜利；额外 1000-seed 发布探针同样无失败。`npm run test:browser` 使用真实 Chromium 验证星域界面的视口滚动、指挥官招募、真实航行、次级前哨建立、本地建设队列、移动敌军、主基地围攻、舰队回防、D.1 船坞生产、D.2 逐舰撤离，以及 D.3 遗迹蓝图取得、跨域激活和效果说明；完整三星域 UI 流程仍完成六次 Three.js 战斗和胜利结算。也可通过 `BROWSER_TEST_URL` 验收已经启动的单文件静态站点：
...
- alpha.2 的零值、极低值和高值迁移保持确定、合法并按最近可达战力钳制；Sector Expedition Code 当前为 `1.0-alpha.12`。
...
V1.0-D.3 将三个永久蓝图接入统一战略效果表，并严格区分“本星域回收”与“成功撤离后激活”。后勤影响燃料派生量，工业影响正式设施/生产扣费，强化舰体只影响战略撤离风险；不会改写 core-v4 舰船定义、组件 HP、成本或战斗价值。Sector Expedition Code 为 `1.0-alpha.12`，策略套件现为 91 项。

下一切片将评估并实现受冻结边界约束的舰船模块装配：若无法在不修改 core-v4 规则/平衡的前提下形成真实闭环，将明确限制为战略层装备，不伪造战斗加成。后续仍不默认扩展多玩家舰队、基地升级树、实时战略移动、外交或市场。
