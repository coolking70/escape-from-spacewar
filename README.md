...
- `core-v4` 单场 3D 太空战斗；
- V0.6～V0.9 的七层航道兼容战役；
- V1.0「单星域高速 SLG + 星门撤离」主方向（V1.0-C.5 已完成三星域真实玩家闭环与发布收尾）。
...
- 版本：`1.0-alpha.9`（真实逐舰舰队、完整指挥官闭环、多据点、移动敌军、围攻、唯一星门防御战与三星域闭环）；`1.0-alpha.2` 至 `1.0-alpha.8` 存档均会确定性迁移到当前版本
- 保存当前星域、实体、固定驻军、移动特遣舰队、围攻、设施、队列、危机、星门防御、真实舰队与跨域继承状态
...
`npm run test:strategy` 覆盖 81 项（无 `as unknown as` 伪造 BattleState），其中 C.5 正式矩阵对 65 个 seed 逐步执行远征码往返与六场真实 core-v4 战斗并全部取得三星域胜利；额外 1000-seed 发布探针同样无失败。`npm run test:browser` 使用真实 Chromium 验证星域界面的视口滚动、指挥官招募、真实航行、次级前哨建立、本地建设队列、移动敌军、主基地围攻、舰队回防，以及完整三星域 UI 流程中的六次 Three.js 战斗和胜利结算；也可通过 `BROWSER_TEST_URL` 验收已经启动的单文件静态站点：
...
- alpha.2 的零值、极低值和高值迁移保持确定、合法并按最近可达战力钳制；Sector Expedition Code 当前为 `1.0-alpha.9`。
...
V1.0-C.5 已完成三星域真实玩家闭环与压力收尾：每域建立基地、航路研究、招募决定、raider 围攻/战斗、星门测绘、校准、防御和紧急撤离都通过正式 reducer、Sector Expedition Code、core-v4、Three.js 与逐舰写回。策略套件现为 81 项；65-seed 正式矩阵与额外 1000-seed 探针全部胜利，Chromium 通过可见按钮完成三个星域和六场真实战斗。
...
V1.0-C 功能范围至此冻结。下一步应先完成当前分支的提交、PR 与发布候选验收；只有另行批准后才进入 V1.0-D，且仍不默认扩展多玩家舰队、实时战略移动、外交或市场。
