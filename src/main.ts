...
(window as unknown as { runBalanceTest: () => unknown }).runBalanceTest = runBalanceTest;
(window as unknown as { runBalance: (config?: BalanceRunConfig) => unknown }).runBalance = runBalance;
(window as unknown as { runAcceptanceTests: () => unknown }).runAcceptanceTests = runAcceptanceTests;
(window as unknown as { runSimulationStressTest: () => unknown }).runSimulationStressTest = runSimulationStressTest;
(window as unknown as { runCampaignTests: () => unknown }).runCampaignTests = runCampaignTests;
(window as unknown as { loadReplay: (code: string) => void }).loadReplay = (code: string) => {
  const input = document.querySelector('#replayCodeInput') as HTMLTextAreaElement | null;
  if (input) input.value = code;
};

installCampaignRuntimeControls();
new App();
