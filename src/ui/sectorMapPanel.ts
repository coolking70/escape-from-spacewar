...
export class SectorMapPanel {
  ...
  render(state: CampaignState): void {
    ...
    const actions = state.pendingBattle ? `<button class="btn danger" id="sp-battle">解决战斗：${state.pendingBattle.reason}</button>` : `<button class="btn" id="sp-scan" ${disabled(true)}>扫描</button><button class="btn" id="sp-gather" ${disabled(current.type === 'resource' && !current.gathered)}>采集</button><button class="btn" id="sp-signal" ${disabled(current.type === 'signal' && !current.signalResolved)}>处理信号</button><button class="btn primary" id="sp-gate" ${disabled(current.type === 'gate')}>进入星门</button><button class="btn" id="sp-wait" ${disabled(true)}>等待</button>`;
    ...
    click('#sp-scan', { type: 'scan' }); click('#sp-gather', { type: 'gather' }); click('#sp-signal', { type: 'resolveSignal', optionId: 'investigate' }); click('#sp-gate', { type: 'enterGate' }); click('#sp-wait', { type: 'wait' });
