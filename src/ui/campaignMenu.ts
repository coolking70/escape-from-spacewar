...
export class CampaignMenu {
  ...
  show(): void {
    ...
    (this.root.querySelector('#cm-new') as HTMLButtonElement).onclick = () => {
      const name = (this.root.querySelector('#cm-name') as HTMLInputElement).value.trim() || '星域指挥官';
      const focus = (this.root.querySelector('#cm-focus') as HTMLSelectElement).value as CommanderFocus;
      this.cb.onNew({ name, focus });
    };
