...
export class CampaignMenu {
  ...
  show(): void {
    ...
    this.root.innerHTML = `<div class="campaign-menu"><h1>Escape from SpaceWar</h1><p>单场战斗、旧版路线远征与新星域搜打撤 SLG</p><div class="strategic-create"><h2>星域战略远征 · V1.0 重构</h2><p>每个星域都是一局高速微型 SLG：探索、抢占据点、建设、科研、打击敌军，并在危机彻底爆发前穿过星门。</p><label>远征组织名称 <input id="cm-strategy-name" maxlength="32" value="深空远征团"></label><button class="btn primary" id="cm-strategy-new">开始星域战略远征</button><button class="btn" id="cm-strategy-continue" ${this.cb.hasStrategicSave() ? '' : 'disabled'}>继续星域战略远征</button><textarea id="cm-strategy-import" placeholder="粘贴 Sector Expedition Code"></textarea><button class="btn" id="cm-strategy-import-btn">导入星域远征码</button></div><div class="commander-create"><h2>旧版 FTL 式星域战役</h2><label>指挥官姓名 <input id="cm-name" maxlength="24" value="星域指挥官"></label><label>初始专长 <select id="cm-focus">${focuses}</select></label><small>保留七层航道、节点事件、战斗、打捞、指挥官与组织科技系统，作为兼容模式。</small></div><div class="organization-create"><h2>创建旧版远征组织</h2><label>组织名称 <input id="cm-org-name" maxlength="32" value="深空远征团"></label><label>组织原型 <select id="cm-org-archetype">${archetypes}</select></label><label>政体 <select id="cm-org-government">${governments}</select></label><div class="organization-values"><label>价值观一 <select id="cm-org-value-a">${values}</select></label><label>价值观二 <select id="cm-org-value-b">${values}</select></label></div></div><button class="btn" id="cm-new">开始旧版星域战役</button><button class="btn" id="cm-continue" ${this.cb.hasSave() ? '' : 'disabled'}>继续旧版战役</button><textarea id="cm-import" placeholder="粘贴 Campaign Code"></textarea><button class="btn" id="cm-import-btn">导入旧版战役码</button><button class="btn" id="cm-single">单场战斗</button></div>`;
    ...
    (this.root.querySelector('#cm-strategy-new') as HTMLButtonElement).onclick = () =>
      this.cb.onStrategicNew((this.root.querySelector('#cm-strategy-name') as HTMLInputElement).value.trim() || '深空远征团');
